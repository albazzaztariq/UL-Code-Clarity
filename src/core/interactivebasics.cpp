...
void InteractiveBasicsDialog::buildUI()
{
    ...
    QJsonArray topics = JsonLoader::loadArray("tutorials.json", "interactive_topics");
...
// ============================================================================
// Generic topic page builder — loads from JSON object
// ============================================================================
...
QWidget* InteractiveBasicsDialog::buildTopicPage(const QJsonObject &topic, int stepIndex)
{
    ...
    // Special note for Objects & Inheritance (id == 6): C doesn't have classes
    if (topic["id"].toInt() == 6) {
        ...
        auto *ntxt = new QLabel(
            ...
            "This topic shows Python examples. If you selected C only, the concept of "
            "grouping data still applies.",
            note);
    ...
    for (int i = 0; i < exercises.size(); ++i) {
        ...
        connect(ew.checkBtn, &QPushButton::clicked,
                this, &InteractiveBasicsDialog::onCheckAnswerClicked);
        m_topicExercises[stepIndex].exercises.append(ew);

        // Hide all exercises after the first until the previous one is answered
        if (i > 0) {
